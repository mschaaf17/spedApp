                ...
                <Tabs activeKey={activeTab} onChange={setActiveTab} size="large">
                  ...
                  <TabPane tab="Data Measures" key="dataMeasures">
                    <div className="data-measures-content">
                      ...
                      <StudentDataMeasuresTable
                        ...
                        onViewChart={(record) => {
                          // Charts are now integrated into the Data Measures tab
                          // Just refetch student data to ensure we have the latest duration/frequency data
                          if (selectedStudent?.username) {
                            refetchSelectedStudent();
                          }
                        }}
                  ...
                  <TabPane tab="Contracts" key="contracts">
                    <div className="contracts-content">
                      ...
                      {/* Contract Charts */}
                      {selectedStudentData?.user?.contracts?.filter(contract => contract.isActive).length > 0 && (
                        <div style={{ marginTop: 32 }}>
                          <h4>Contract Progress Charts</h4>
                          <div style={{ marginTop: 16 }}>
                            {selectedStudentData.user.contracts
                              .filter(contract => contract.isActive)
                              .map(contract => (
                                <div key={contract._id} style={{ marginBottom: 24 }}>
                                  <ContractCharts contract={contract} />
                                </div>
                              ))}
                          </div>
                        </div>
                  ...
                  {selectedStudentData?.user?.breakSettings?.isEnabled && (
                    <TabPane tab="Breaks" key="breaks">
                      ...
                      {/* Break Charts */}
                      {selectedStudentData?.user?.breakHistory && selectedStudentData.user.breakHistory.length > 0 && (
                        <div style={{ marginTop: 32 }}>
                          <h4>Break Data Charts</h4>
                          <div style={{ marginTop: 16 }}>
